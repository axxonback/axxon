...
function showArticlesPost($id) {
    $r = redisLink();
    $post = $r->hgetall("articles_post:$id");
    if (empty($post)) return false;

    $userid = $post['user_id'];
    $username = $r->hget("user:$userid","username");
    $elapsed = strElapsed($post['time']);
//    $userlink = "<a class=\"username\" href=\"profile.php?u=".urlencode($use    rname)."\">".utf8entities($username)."</a>";

    
    echo('<div class="post">'.$post['articles_body']."<br>");
    echo('<i>posted '.$elapsed.' ago via web</i></div>');
...
function showArticlesUserPosts($userid,$start,$count) {
    $r = redisLink();
//    $key = ($userid == -1) ? "articles" : "articles_posts:$userid";
    $key = "articles_posts:$userid";
    $posts = $r->lrange($key,$start,$start+$count);
    $c = 0;
    foreach($posts as $p) {
        if (showArticlesPost($p)) $c++;
        if ($c == $count) break;
    }
    return count($posts) == $count+1;
}
...
/*function showArticlesUserPostsWithPagination($username,$userid,$start,$count) {
    global $_SERVER_1;
    $thispage = $_SERVER_1['PHP_SELF'];
    ...
    $u = $username ? "&u=".urlencode($username) : "";
    if (showArticlesUserPosts($userid,$start,$count))
        $nextlink = "<a href=\"$thispage?start=$next".$u."\">Older posts &ra    quo;</a>";
    if ($start > 0) {
        $prevlink = "<a href=\"$thispage?start=$prev".$u."\">&laquo; Newer p    osts</a>".($nextlink ? " | " : "");
    }
    if ($nextlink || $prevlink)
        echo("<div class=\"rightlink\">$prevlink $nextlink</div>");
}*/
